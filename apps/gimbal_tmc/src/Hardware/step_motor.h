...
/*
 * TMC2209 步进驱动（STEP/DIR 独立模式）
 *
 * 接线（与 apps/gimbal 引脚对齐）
 ...
 *
 * 硬件注意：
 *   PDN_UART 接高电平（独立 STEP/DIR 模式，禁用 UART）
 *   CLK 悬空使用内部时钟
 *   VIO 接 3.3V，VM 接电机电源
 *
 * 细分表 (MS1 / MS2)：
 *   1/8  Low  Low
 *   1/16 High High
 *   1/32 High Low   ← 默认（200 步电机 → 6400 pulse/rev）
 *   1/64 Low  High
 *
...
typedef enum {
    TMC_MICROSTEP_1_8  = 0, /* MS1=L MS2=L */
    TMC_MICROSTEP_1_16 = 1, /* MS1=H MS2=H */
    TMC_MICROSTEP_1_32 = 2, /* MS1=H MS2=L  default */
    TMC_MICROSTEP_1_64 = 3  /* MS1=L MS2=H */
} tmc_microstep_t;

void step_motor_init(void);

/* EN 低有效：enable=1 使能驱动，enable=0 关断 */
